...
// READ ROLE (default: viewer)
$role = $_SESSION["admin_role"] ?? "viewer";

// ROLE RULES:
$canManage = ($role === "admin" || $role === "superadmin");
$canViewResponses = ($role === "superadmin");

$username = ucfirst($_SESSION["admin_username"] ?? "Admin");
?>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <title><?= htmlspecialchars($username) ?> Dashboard — Survey Manager</title>
    ...
    <style>
        /* Disable class for restricted access */
        .disabled {
            pointer-events: none;
            opacity: 0.4;
            user-select: none;
...
<body>
    <div class="app">
        <aside class="sidebar">
            <div class="brand">Survey Panel — <?= htmlspecialchars($username) ?> </div>
            <nav class="nav">
                <button id="nav-dashboard" class="active">Dashboard</button>
                <button id="nav-analytics">Analytics</button>

                <!-- Manage Survey — restricted -->
                <button id="nav-manage" class="<?= $canManage ? '' : 'disabled' ?>">Manage Survey</button>

                <!-- Responses — restricted -->
                <button id="nav-responses" class="<?= $canViewResponses ? '' : 'disabled' ?>">Responses</button>
        ...
        <main class="main">
            <header class="topbar">
                <div class="top-left">Customer Satisfaction Survey</div>
            ...
            <section id="page-dashboard">
                ...
                <div class="card">
                    <h3>Overall Average Trend</h3><canvas id="lineChart" height="120"></canvas>
                </div>
                <div class="card">
                    <h3>Category Contribution</h3><canvas id="pieChart" height="120"></canvas>
                </div>
            </section>
            ...
            <!-- MANAGE (restricted) -->
            <section id="page-manage" style="display:none" class="<?= $canManage ? '' : 'disabled' ?>">
                <div class="card">
            ...
            <!-- RESPONSES (restricted) -->
            <section id="page-responses" style="display:none" class="<?= $canViewResponses ? '' : 'disabled' ?>">
